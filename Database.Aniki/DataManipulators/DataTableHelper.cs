    ...
    public static class DataTableHelper
    {
        ...
        public static List<T> DataTableToList<T>(DataTable dt) where T : class, new()
        {
            try
            {
                ...
                lock (Shared._ObjectPropertiesCache)
                {
                    if (!Shared._ObjectPropertiesCache.TryGetValue(typeFromHandle, out value))
                    {
                        value = new List<PropertyInfo>(typeFromHandle.GetProperties());
                        value.RemoveAll((PropertyInfo item) => !item.CanWrite);
                        Shared._ObjectPropertiesCache.Add(typeFromHandle, value);
                    }
                }
